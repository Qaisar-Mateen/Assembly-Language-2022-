...
mov [es:160], al ; write at top left of screen
...
mov [es:318], al ; write at top right of screen
inc bx ; increment to next shape
and bx, 3 ; taking modulus by 4
...
;---------------------------------------------------------------------------
taskthird: mov al, [chars+bx] ; read the next shape...0
mov [es:3200], al ; write at top right of screen
inc bx ; increment to next shape
and bx, 3 ; taking modulus by 4
jmp taskthird ; infinite task

;---------------------------------------------------------------------------
; fourth task to be multitasked
;---------------------------------------------------------------------------
taskfourth: mov al, [chars+bx] ; read the next shape...0
mov [es:3358], al ; write at top right of screen
inc bx ; increment to next shape
and bx, 3 ; taking modulus by 4
jmp taskfourth; infinite task
...
mov word [pcb+10+4], taskthird ; initialize ip
mov [pcb+10+6], cs ; initialize cs
mov word [pcb+10+8], 0x0200 ; initialize flags
mov word [pcb+20+4], taskfourth ; initialize ip
mov [pcb+20+6], cs ; initialize cs
mov word [pcb+20+8], 0x0200 ; initialize flags
